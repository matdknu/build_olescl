#!/bin/bash
# Script para agregar logo a todos los headers
...
update_logo() {
    ...
    if grep -q "<h1><a href.*Observatorio de Legitimidad</a></h1>" "$file" || grep -q "<h1>Observatorio de Legitimidad</h1>" "$file" || grep -q 'style="text-decoration: none; color: white;">' "$file"; then
        # Reemplazar diferentes variaciones
        sed -i '' "s|<a href=\"\([^\"]*\)\" style=\"text-decoration: none; color: white;\"><h1 style=\"margin: 0; font-size: 1.5rem;\">Observatorio de Legitimidad</h1></a>|<a href=\"\1\" style=\"display: flex; align-items: center; gap: 0.75rem; text-decoration: none; color: white;\"><img src=\"${base_path}logos/logo.png\" alt=\"Observatorio de Legitimidad\" style=\"height: 45px; width: auto; object-fit: contain; filter: brightness(1.1);\"><h1 style=\"margin: 0; font-size: 1.5rem; font-weight: 600;\">Observatorio de Legitimidad</h1></a>|g" "$file"
        sed -i '' "s|<h1><a href=\"\([^\"]*\)\" style=\"color: white; text-decoration: none;\">Observatorio de Legitimidad</a></h1>|<a href=\"\1\" style=\"display: flex; align-items: center; gap: 0.75rem; text-decoration: none; color: white;\"><img src=\"${base_path}logos/logo.png\" alt=\"Observatorio de Legitimidad\" style=\"height: 45px; width: auto; object-fit: contain; filter: brightness(1.1);\"><h1 style=\"margin: 0; font-size: 1.5rem; font-weight: 600;\">Observatorio de Legitimidad</h1></a>|g" "$file"
    fi
}
